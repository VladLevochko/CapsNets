...
class CapsuleLoss(Module):
    def __init__(self):
        ...
        self.reconstruction_loss = MSELoss(True)
        self.lambda_reconstruction = 0.001

    def forward(self, y_predicted, y_true, reconstruction, image):
        return self.margin_loss(y_predicted, y_true) \
               + self.lambda_reconstruction * self.reconstruction_loss(reconstruction, image)
