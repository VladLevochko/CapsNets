...
from reid.center_loss import CenterLoss
from trainer import Trainer


class ReidTrainer(Trainer):
    def __init__(self, model, train_loader, test_loader, lfw_loader, writer, device="cpu", lr=0.1, lr_decay=0.001,
                 classes_number=10, embedding_size=256, lamda=0.01, alpha=0.03, **kwargs):
        super().__init__(train_loader, test_loader, writer, device, lr, lr_decay, test_interval=0, **kwargs)

        self.lfw_loader = lfw_loader

        self.model = model.to(device)
        self.center_loss = CenterLoss(num_classes=classes_number, feat_dim=embedding_size, device=device, alpha=alpha).to(device)
        self.cross_entropy_loss = CrossEntropyLoss().to(device)

        # self.optimizer_model = SGD(self.model.parameters(), lr=lr, weight_decay=lr_decay)
        self.optimizer_model = SGD(self.model.parameters(), lr=lr)
    ...
    def train_step(self, epoch):
        ...
        for i, (images, labels) in enumerate(tqdm(self.train_loader, desc="Training")):
            images, labels = images.to(self.device), labels.to(self.device)

            self.optimizer_model.zero_grad()

            embeddings, predictions = self.model(images)

            l_center = self.center_loss(embeddings, labels)
            l_cross_entropy = self.cross_entropy_loss(predictions, labels)
            loss = l_cross_entropy + self.lamda * l_center
            ...
            cross_entropy_loss += l_cross_entropy.item()
            center_loss += l_center.item()
        ...
        self.writer.add_scalar("Loss/CrossEntropy", cross_entropy_loss, epoch)
        self.writer.add_scalar("Loss/Center", center_loss, epoch)

        print("Train loss {:.4} cel {:.4} cl {:.4} accuracy {:.4}".format(total_loss, cross_entropy_loss, center_loss, epoch_accuracy))
    ...
    def val_step(self, epoch):
        ...
        for images, _ in tqdm(self.lfw_loader, desc="Validation"):
            images = images.to(self.device)
            embedding, _ = self.model(images)
            embeddings.extend(embedding.tolist())

        pairs = self.lfw_loader.dataset.get_pairs()
        embeddings1 = []
        embeddings2 = []
        issame = []
        for id1, id2, equals in pairs:
            embeddings1.append(embeddings[id1])
            embeddings2.append(embeddings[id2])
            issame.append(equals)
        embeddings1 = np.array(embeddings1)
        embeddings2 = np.array(embeddings2)
        issame = np.array(issame)
        distances = self.distance(embeddings1, embeddings2, 1)

        result = self.evaluate_lfw(distances, issame)

        self.writer.add_scalar("Accuracy/Val", np.mean(result[4]), epoch)
        print("Val accuracy {:.4}+-{:.4}".format(np.mean(result[4]), np.std(result[4])))
    ...
    def evaluate_lfw(self, distances, labels, num_folds=10, far_target=1e-3):
